...
try:
    ...
    def get_player_id(player_name):
        p.execute("USE AURORA;")
        player_id = execute_sql(
            f'SELECT PLAYER_ID FROM game_stats WHERE PLAYER_NAME = "{player_name}"'
        )[0][0]
        return player_id
    ...
    def get_player_info(info_type, player_name):
        p.execute("USE AURORA;")
        info = execute_sql(
            f'SELECT {info_type} FROM game_stats WHERE PLAYER_NAME = "{player_name}"'
        )[0][0]
        return info
    ...
    def is_unlocked(player_name, name, a_type):
        ...
        if a_type == "spell":
            ...
            return
        data_set = execute_sql(
            f'SELECT {name} FROM {a_type} WHERE PLAYER_ID = "{get_player_id(player_name)}";'
        )
    ...
    def get_troop(troop_name, player_id):
        ...
        data = execute_sql(
            f'SELECT * FROM {troop_name} WHERE PLAYER_ID  = "{player_id}";'
        )[0]
        return {
            "attack": data[1],
            "defence": data[2],
    ...
    def get_spell(spell, player_id):
        execute_sql("USE AURORA;")
        data = execute_sql(
            f'SELECT {spell} FROM spells WHERE PLAYER_ID  = "{player_id}";'
        )[0][0]
        return data
    ...
    def update(table_name, column, value, player_id):
        p.execute("USE AURORA;")
        p.execute(
            f'UPDATE {table_name} SET {column} = {value} WHERE PLAYER_ID = "{player_id}";'
        )
        db.commit()
    ...
    def create_player(player_name):
        ...
        p.execute(
            f'INSERT INTO game_stats (PLAYER_ID, PLAYER_NAME) VALUES ("{player_id}", "{player_name.upper()}") ;'
        )
    ...
    if __name__ == "__main__":
        execute_sql_from_file(getcwd() + "\\SQL\\script001.sql")
